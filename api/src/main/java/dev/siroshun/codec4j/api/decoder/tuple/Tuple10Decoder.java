...
record Tuple10Decoder<T, V1, V2, V3, V4, V5, V6, V7, V8, V9, V10>(Function10<V1, V2, V3, V4, V5, V6, V7, V8, V9, V10, T> constructor,
                                                                  ...
                                                                  TupleValueDecoder<V7> decoder7,
                                                                  TupleValueDecoder<V8> decoder8,
                                                                  TupleValueDecoder<V9> decoder9,
                                                                  TupleValueDecoder<V10> decoder10) implements Decoder<T> {
    ...
    public @NotNull Result<T, DecodeError> decode(@NotNull In in) {
        ...
        Result<V8, DecodeError> v8Result = this.decoder8.decodeFromElementReader(reader, 7);
        if (v8Result.isFailure()) {
            return v8Result.asFailure();
        }

        Result<V9, DecodeError> v9Result = this.decoder9.decodeFromElementReader(reader, 8);
        if (v9Result.isFailure()) {
            return v9Result.asFailure();
        }

        Result<V10, DecodeError> v10Result = this.decoder10.decodeFromElementReader(reader, 9);
        if (v10Result.isFailure()) {
            return v10Result.asFailure();
        }
        ...
        return Result.success(this.constructor.apply(v1Result.unwrap(), v2Result.unwrap(), v3Result.unwrap(), v4Result.unwrap(), v5Result.unwrap(), v6Result.unwrap(), v7Result.unwrap(), v8Result.unwrap(), v9Result.unwrap(), v10Result.unwrap()));
    }
}
