...
public final class MapCodec {
     ...
     */
    @Contract("_, _ -> new")
    public static <K, V> @NotNull Codec<Map<K, V>> create(@NotNull Codec<K> keyCodec, @NotNull Codec<V> valueCodec) {
        return Codec.codec(
            MapEncoder.create(keyCodec, valueCodec),
            MapDecoder.create(keyCodec, valueCodec)
    ...
    public static <K, V> @NotNull Codec<Map<K, V>> create(@NotNull Codec<K> keyCodec, @NotNull Codec<V> valueCodec,
        ...
        return Codec.codec(
            MapEncoder.create(keyCodec, valueCodec),
            MapDecoder.create(keyCodec, valueCodec, factory, allowDuplicatedKey, finalizer)
        ).named("MapCodec[key=" + keyCodec + ",value=" + valueCodec + "]");
    }
