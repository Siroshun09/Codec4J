...
record Tuple3Decoder<T, V1, V2, V3>(Function3<V1, V2, V3, T> constructor,
                                    TupleValueDecoder<V1> decoder1,
                                    TupleValueDecoder<V2> decoder2,
                                    TupleValueDecoder<V3> decoder3) implements Decoder<T> {
    ...
    public @NotNull Result<T, DecodeError> decode(@NotNull In in) {
        ...
        return Result.success(this.constructor.apply(v1Result.unwrap(), v2Result.unwrap(), v3Result.unwrap()));
    }
}
