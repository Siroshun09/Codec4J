...
record Tuple5Decoder<T, V1, V2, V3, V4, V5>(Function5<V1, V2, V3, V4, V5, T> constructor,
                                            TupleValueDecoder<V1> decoder1,
                                            TupleValueDecoder<V2> decoder2,
                                            TupleValueDecoder<V3> decoder3,
                                            TupleValueDecoder<V4> decoder4,
                                            TupleValueDecoder<V5> decoder5) implements Decoder<T> {
    ...
    public @NotNull Result<T, DecodeError> decode(@NotNull In in) {
        ...
        return Result.success(this.constructor.apply(v1Result.unwrap(), v2Result.unwrap(), v3Result.unwrap(), v4Result.unwrap(), v5Result.unwrap()));
    }
}
